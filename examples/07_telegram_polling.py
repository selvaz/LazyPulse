...
pulse = PulseAgent(
    name="tg-assistant",
    engine=LLMEngine("claude-opus-4-7", system="You are a concise personal assistant."),
    # Reply freely, but only ever to the owner's chat (allow-list bound).
    tools=[TelegramTools(client, allowed_chat_ids=[OWNER_ID], require_confirmation=False)],
